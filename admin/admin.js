...
async function loadStats() {
  ...
  const partnersCount = await getCount("partners");

  const kycCount = await getCount("kyc");
  ...
  const totalKYC =
    document.getElementById("totalKYC");
  ...
  if (totalBookings) {
    totalBookings.innerText = bookingsCount;
  }

  if (totalPartners) {
    totalPartners.innerText = partnersCount;
  }

  if (totalKYC) {
    totalKYC.innerText = kycCount;
  }
